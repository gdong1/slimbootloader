...
#
# This script extends GenCfgData.py with a GENUIB command that produces
# a UiDesc.bin file containing page/field/option/condition/string metadata
# for the runtime UiSetup TUI.
#
# Usage:
#   python GenUiDescBin.py <CfgDataDef.yaml> <UiDesc.bin>
#   python GenUiDescBin.py <CfgDataDef.yaml> <UiDescData.h>  (if .h extension)
#
...
#
# Add GenCfgData.py's directory to path
#
        ...
        for item in cfg_list:
            ...
            if item['path'].startswith(tag_name + '.') or ('.' + tag_name + '.') in item['path']:
                if cname == field_name:
                    ...
                    # We need offset from after the header
                    bit_offset = item['offset'] % (item['length'] + item['offset'])
