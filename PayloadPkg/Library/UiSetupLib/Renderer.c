  ...
  )
{
  ...
  PutString (LegendRow + 1, 2, "I/K/J/L=Nav  Enter=Edit  +/-=Change  X=Exit  Q/F9=Defaults  S/F10=Save  H=Help",
             ATTR_LEGEND, mCols - 4);
  ...
  )
{
  ...
  switch (Key) {
    case UI_KEY_UP:
      AsciiSPrint (Buf, BufSize, "UP");
      break;
    case UI_KEY_DOWN:
      AsciiSPrint (Buf, BufSize, "DOWN");
      break;
    case UI_KEY_LEFT:
      AsciiSPrint (Buf, BufSize, "LEFT");
      break;
    case UI_KEY_RIGHT:
      AsciiSPrint (Buf, BufSize, "RIGHT");
      break;
    case UI_KEY_HOME:
      AsciiSPrint (Buf, BufSize, "HOME");
      break;
    case UI_KEY_END:
      AsciiSPrint (Buf, BufSize, "END");
      break;
    case UI_KEY_PGUP:
      AsciiSPrint (Buf, BufSize, "PGUP");
      break;
    case UI_KEY_PGDN:
      AsciiSPrint (Buf, BufSize, "PGDN");
      break;
    case UI_KEY_ENTER:
      AsciiSPrint (Buf, BufSize, "ENTER");
      break;
    case UI_KEY_ESC:
      AsciiSPrint (Buf, BufSize, "ESC");
      break;
    case UI_KEY_TAB:
      AsciiSPrint (Buf, BufSize, "TAB");
      break;
    case UI_KEY_BKSP:
      AsciiSPrint (Buf, BufSize, "BKSP");
      break;
    case UI_KEY_PLUS:
      AsciiSPrint (Buf, BufSize, "+");
      break;
    case UI_KEY_MINUS:
      AsciiSPrint (Buf, BufSize, "-");
      break;
    case UI_KEY_QUIT:
      AsciiSPrint (Buf, BufSize, "QUIT");
      break;
    case UI_KEY_NONE:
      AsciiSPrint (Buf, BufSize, "NONE");
      break;
    default:
      if ((Key >= 0x20) && (Key < 0x7F)) {
        AsciiSPrint (Buf, BufSize, "'%c'", (CHAR8)Key);
      } else {
        AsciiSPrint (Buf, BufSize, "0x%04X", Key);
      }
      break;
  }
  ...
  )
{
  ...
  // Value
  if (IsHex) {
    AsciiSPrint (ValStr, sizeof(ValStr), "0x%08X", CurrentValue);
  } else {
    AsciiSPrint (ValStr, sizeof(ValStr), "%d", CurrentValue);
  }
  PutString (StartRow + 3, StartCol + 2, "Value: ", ATTR_POPUP, 7);
  PutString (StartRow + 3, StartCol + 9, ValStr, ATTR_POPUP_SEL, PopupWidth - 12);

  // Instructions
  PutString (StartRow + 4, StartCol + 2, "+/-=Inc/Dec  Enter=OK  Esc=Cancel", ATTR_POPUP, PopupWidth - 4);
}
  ...
  )
{
  ...
  for (Row = 0; Row < SerialRows; Row++) {
    ...
    if (SerialCols > 0) {
      SerialPortWrite ((UINT8 *)&Line[SegStart], SerialCols - SegStart);
    }
