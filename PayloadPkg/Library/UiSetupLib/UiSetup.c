  ...
  )
{
  ...
  return RendererGetRows () - UI_TITLE_ROWS - UI_HELP_ROWS - UI_LEGEND_ROWS - 3;
}
  ...
  while (TRUE) {
    ...
    switch (Key) {
      ...
      case UI_KEY_PGUP:
        DumpHighNibblePending = FALSE;
        if (DumpMode && (ElemBits == 8)) {
          PageBytes = (UINT16)(BytesPerRow * VisibleRows);
          if (ElemIdx > PageBytes) {
            ElemIdx = (UINT16)(ElemIdx - PageBytes);
          } else {
            ElemIdx = 0;
          }
          TopIdx = (UINT16)((ElemIdx / BytesPerRow) * BytesPerRow);
        } else {
          if (TopIdx > VisibleRows) {
            TopIdx = (UINT16)(TopIdx - VisibleRows);
          } else {
            TopIdx = 0;
          }
          ElemIdx = TopIdx;
      ...
      case UI_KEY_PGDN:
        ...
        if (DumpMode && (ElemBits == 8)) {
          PageBytes = (UINT16)(BytesPerRow * VisibleRows);
          if ((UINT16)(ElemIdx + PageBytes) < ElemCount) {
            ElemIdx = (UINT16)(ElemIdx + PageBytes);
          } else {
            ElemIdx = (UINT16)(ElemCount - 1);
          }
          TopIdx = (UINT16)((ElemIdx / BytesPerRow) * BytesPerRow);
        } else {
          if ((UINT16)(TopIdx + VisibleRows) < ElemCount) {
            TopIdx = (UINT16)(TopIdx + VisibleRows);
            if (TopIdx >= ElemCount) {
              TopIdx = (UINT16)(ElemCount - 1);
            }
            ElemIdx = TopIdx;
          }
        }
    ...
    switch (State) {
      ...
      case UI_STATE_PAGE:
        ...
        switch (Key) {
          ...
          case UI_KEY_PLUS:
            //
            // Quick increment for current field
            //
            CurField = GetVisibleField (CurPage, ScrollOffset + SelField, CfgBuf);
            if (CurField != NULL) {
              TagData = CfgGetTagData (CurField->TagId);
              if (TagData != NULL) {
                UINT32 Val = FormGetFieldValue (CurField, (UINT8 *)TagData);
                if (CurField->FieldType == UI_FIELD_TYPE_COMBO) {
                  //
                  ...
                  UINT16 OptCnt;
                  UINT16 OIdx;
                  Opts = FormGetOptions (CurField, &OptCnt);
                  for (OIdx = 0; OIdx < OptCnt; OIdx++) {
                    if (Opts[OIdx].Value == Val) {
                      OIdx = (OIdx + 1) % OptCnt;
                      FormSetFieldValue (CurField, (UINT8 *)TagData, Opts[OIdx].Value);
                      CfgRecordFieldChange (CurField, Opts[OIdx].Value);
                      CfgSetDirty ();
                      break;
                    }
                  }
                } else if (CurField->FieldType == UI_FIELD_TYPE_TABLE) {
                  // Table fields are edited through HandleTableEdit() on Enter.
                } else {
                  UINT64 Val64 = ReadFieldValue64 (CurField, (UINT8 *)TagData);
                  UINT64 MaxV = (CurField->BitLength >= 64) ? MAX_UINT64 : (LShiftU64 (1ULL, CurField->BitLength) - 1ULL);
                  if (Val64 < MaxV) {
                    WriteFieldValue64 (CurField, (UINT8 *)TagData, Val64 + 1);
                    RecordFieldChange64 (CurField, Val64 + 1);
                    CfgSetDirty ();
                  }
                }
              }
          ...
          case UI_KEY_MINUS:
            //
            // Quick decrement for current field
            //
            CurField = GetVisibleField (CurPage, ScrollOffset + SelField, CfgBuf);
            if (CurField != NULL) {
              TagData = CfgGetTagData (CurField->TagId);
