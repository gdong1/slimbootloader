  ...
  )
{
  ...
  if (TagId == CDATA_FSPM_UPD_TAG) {
    FspUpd = (LoaderFspInfo != NULL) ? (UINT8 *)(UINTN)LoaderFspInfo->FspmUpdPtr : NULL;
    if (FspUpd != NULL) {
      return FspUpd + sizeof (FSPM_UPD_COMMON_FSP24);
    }
  } else if (TagId == CDATA_FSPS_UPD_TAG) {
    FspUpd = (LoaderFspInfo != NULL) ? (UINT8 *)(UINTN)LoaderFspInfo->FspsUpdPtr : NULL;
    if (FspUpd != NULL) {
      if (!mFspsUpdPtrFoundLogged) {
        DEBUG ((
          DEBUG_INFO,
          "UiSetup: FSPS data available via LoaderFspInfo HOB cfg=%p\n",
          FspUpd + sizeof (FSPS_UPD_COMMON_FSP24)
          ));
        mFspsUpdPtrFoundLogged = TRUE;
      }

      return FspUpd + sizeof (FSPS_UPD_COMMON_FSP24);
    }

    if (!mFspsUpdPtrMissingLogged) {
      DEBUG ((
        DEBUG_WARN,
        "UiSetup: FSPS tag 0x%03X has no data (FindConfigDataByTag miss, LoaderFspInfo/FspsUpdPtr is NULL)\n",
        TagId
        ));
