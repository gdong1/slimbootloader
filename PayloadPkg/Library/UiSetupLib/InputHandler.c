  ...
  )
{
  ...
  SerialPortWrite ((UINT8 *)Buf, sizeof (Buf));
}
...
/**
  ...
  Stops immediately on any non-CR/LF byte (which is consumed but lost; this
  is an acceptable edge case for interactive telnet use).
**/
STATIC
VOID
DrainCrLf (
